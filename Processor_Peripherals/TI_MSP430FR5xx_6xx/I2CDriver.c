 ...
 *					interrupts.
 *
 *	Notes:		(1) External pull-ups are required...for some MSP's the GPIO's
 *					internal pull-ups aren't usable when in I2C mode.  Here's
 *					the schematic for the test function:
 *
 *					                     /|\  /|\
 *					    MSP430FR5869     10k  10k       Slave
 *					       master         |    |        Device
 *					  -----------------   |    |   -----------------
 *					-|XIN  P1.6/UCB0SDA|<-|----+->|SDA              |
 *					 |                 |  |       |                 |
 *					-|XOUT             |  |       |                 |
 *					 |     P1.7/UCB0SCL|<-+------>|SCL              |
 *					 |                 |          |                 |
 *
 *				(2) The acts of (1) reading from UCBxRXBUF, (2) setting UCTXSTT
...
#include "hal_I2C.h"					// header for this module
...
#define I2CREG_STAT			0x0008		// Status Register
#define I2CREG_RXBUF		0x000C		// Receive Buffer
#define I2CREG_TXBUF		0x000E		// Transmit Buffer
#define I2CREG_OA			0x0014		// I2C Own Address
#define I2CREG_SA			0x0020		// I2C Slave Address
#define I2CREG_ICTL			0x002A		// Interrupt Enable Register
#define I2CREG_IE			0x002A		// USCI Bx Interrupt Enable Register
#define I2CREG_IFG			0x002C		// USCI Bx Interrupt Flags Register
#define I2CREG_IV			0x002E		// Interrupt Vector Register
...
#ifdef __MSP430_HAS_EUSCI_B0__			// eUSCI B0
...
i2cResult_t I2CInit(i2cChannel_t channel, i2cConfig_t *configPtr)
{
			...
			(configPtr->mode == I2C_MODE_SLAVE_AND_GEN_CALL))
	{
		...
		REG16(base + I2CREG_CTLW0) = UCSWRST;

		// Clear USCI master mode.
		REG16(base + I2CREG_CTLW0) &= ~UCMST;

		// Configure I2C as Slave and Synchronous mode.
		REG16(base + I2CREG_CTLW0) = UCMODE_3 | UCSYNC;
...
//		REG16(base + I2CREG_OA) = configPtr->address;
	...
	else
	{
		...
		REG16(base + I2CREG_CTLW0) |= UCMST | UCMODE_3 | I2C_CLOCK_SOURCE;

		// If multi-master mode is selected, enable it.
		if (configPtr->mode == I2C_MODE_MULTI_MASTER)
		{
			REG16(base + I2CREG_CTLW0) |= UCMM;
		}
		else
		{
			REG16(base + I2CREG_CTLW0) &= ~UCMM;
		...
		preScalarValue = (uint16_t)(I2C_CLOCK_FREQ_HZ / configPtr->speed);
		REG16(base + I2CREG_BRW) = preScalarValue;

		// Set the address we'll attempt to talk to.
//		REG8(base + I2CREG_SA) = configPtr->address;
	}
...
i2cResult_t I2CWrite(i2cChannel_t channel, uint8_t address, uint8_t *data, uint32_t count)
{
	i2cResult_t result = I2C_RESULT_OK;	// an optimistic return value :)
	uint16_t base = baseAddr[channel];	// Convert channel to base address.
	...
	else
	{
		...
		REG16(base + I2CREG_CTLW0) |= UCTXSTT | UCTR;
...
i2cResult_t I2CRead(i2cChannel_t channel, uint8_t address, uint8_t *data, uint32_t count)
{
	i2cResult_t result = I2C_RESULT_OK;	// an optimistic return value :)
	uint16_t base = baseAddr[channel];	// Convert channel to base address.
...
i2cResult_t I2CWriteThenRead(i2cChannel_t channel, uint8_t address, uint8_t *writeData,
...
i2cResult_t I2CReadThenWrite(i2cChannel_t channel, uint8_t address, uint8_t *readData,
...
uint8_t I2CTest(i2cChannel_t channel)
{
	i2cResult_t result = I2C_RESULT_OK;	// an optimistic return value :)
...
//		config.address = I2C_ADDR;
		...
		// Try a read.
//		result = I2CRead(channel, I2C_ADDR, (uint8_t *)response, sizeof(response));
//		if (result != I2C_RESULT_OK)
//			break;
...
//		while (I2CIsBusy(channel));
		...
		// Try a write.
//		result = I2CWrite(channel, I2C_ADDR, (uint8_t *)writeMsg, sizeof(writeMsg));
//		if (result != I2C_RESULT_OK)
//			break;
...
//		result = I2CWriteThenRead(channel, I2C_ADDR, (uint8_t *)readMsg,
...
static inline void I2C_ISR(i2cChannel_t ch, volatile uint16_t reg)
{
	...
	switch(__even_in_range(reg, USCI_I2C_UCBIT9IFG))	// Find the interrupt's cause.
	{
